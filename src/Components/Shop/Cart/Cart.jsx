...
const Cart = ({cart}) => {
  ...
  for (const product of cart){
    totalPrice = totalPrice + product.data.price;
    shippingCost = shippingCost + product.data.shipping;
  }
  const tax = totalPrice * 7 / 100;
  const grandTotal = totalPrice + shippingCost + tax;
  return (
    <div className="space-y-3 top-0 sticky bg-opacity-50 w-80 bg-pink-200  p-10 font-semibold lg:ml-5 h-fit">
